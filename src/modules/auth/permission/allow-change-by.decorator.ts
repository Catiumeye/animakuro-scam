...
export function AllowChangeBy(decoratorOptions: AllowChangeByDecoratorOptions = {}) {
    return (target: any, propertyName: string) => {
        const validateFn = (target: any, uid: string, requestorScopes: any) => {
          ...
          if (containsAllowedScopes(requestorScopes, decoratorOptions.scopes) ||
          ...
          throw new GqlHttpException(
            `User ${uid} is not allowed to change ${propertyName} field`, HttpStatus.FORBIDDEN
          );
        };

        Reflect.defineMetadata(METADATA_KEY, validateFn, target, propertyName);

        const properties: string[] = Reflect.getMetadata(METADATA_KEY, target) || [];
